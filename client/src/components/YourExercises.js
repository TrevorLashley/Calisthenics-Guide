...
const YourExercises = () => {
  ...
  const fetchExercises = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/exercise-list`);
  ...
  const removeExercise = (e) => {
    ...
    const remove = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/remove-exercise/${id}`, {
          method: "DELETE",
        });
        const data = await response.json();
        console.log(data);
